#!/usr/bin/env python3
"""
Pick & Load 고속 — Doosan M1013

pick_and_load.py와 동일한 동작을 60% 속도로 실행.
속도: 0.3 → 0.6, 가속: 0.3 → 0.6, 스텝 대기: 1.0s → 0.5s

실행:
  ros2 launch dsr_bringup2 dsr_bringup2_gazebo_moveit.launch.py model:=m1013
  python3 /mnt/e/WorkSpace/robot-simulation-platform/robots/doosan-m1013/tasks/pick_and_load_fast.py
"""
import math
import time
import rclpy
...
TASK_DESC = 'Pick & Load 고속'    # 간단 설명

# ── 설정 ──────────────────────────────────────────
MOVE_GROUP_NS = '/sim'           # Gazebo+MoveIt2 네임스페이스
PLANNING_GROUP = 'manipulator'
VELOCITY_SCALE = 0.5             # 50% 속도 (기본 0.3)
ACCEL_SCALE = 0.5                # 50% 가속 (기본 0.3)
STEP_PAUSE = 0.5                 # 스텝 간 대기 (기본 1.0s)
...
class TaskRunner(Node):
    ...
    def run_sequence(self, waypoints):
        ...
        self.get_logger().info(f'  Task Start (FAST) — {total} steps')
        self.get_logger().info(f'  velocity: {VELOCITY_SCALE}, accel: {ACCEL_SCALE}')
        self.get_logger().info('=' * 50)
        ...
        self.get_logger().info('=' * 50)
        self.get_logger().info('  Task Complete (FAST)')
        self.get_logger().info('=' * 50)
        return True
